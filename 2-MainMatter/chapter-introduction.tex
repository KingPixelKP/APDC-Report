...
within the scope of \gls{CDA} at \gls{GMV}. It outlines the context and
motivation for the work carried out, the problem being addressed, the proposed
solution, and the objectives defined for the internship.
...
antenna. Besides emulating currently deployed signals, 
modern simulators can also generate signals corresponding to new 
or upcoming signal generations, allowing receivers and processing algorithms to 
be developed and validated before these signals become operational. \cite{insidegnssVerificationTesting} Such simulators are essential for
...
Additionally, in the context of this internship, an interface that provides the communication channel 
between the developed system and external equipment is developed, enabling remote
command dispatch and status monitoring using \gls{RS232}, a standard for serial communication widely used in industrial and laboratory instrumentation.\cite{wikipediaRS232}
...
Beyond signal generation and transmission, \gls{GNSS} receiver testing also requires
control over auxiliary hardware and external system state, often via
manufacturer-specific serial protocols. The \gls{RS232} interface used in this
context lacks a standard, well-documented application-layer protocol: framing,
message structure, and field encoding are entirely bespoke, and no off-the-shelf
library exists for reliably serializing and deserializing structured data over
this link. Implementing such a protocol from scratch invites the same class of
problems as low-level \gls{DTAPI} usage: manual buffer and framing management,
ad-hoc encoding/decoding logic scattered throughout the codebase, and a high
risk of subtle bugs (e.g. off-by-one errors in frame delimiting, or corruption
from incorrect handling of non-trivially-copyable data) that are difficult to
detect and debug at the byte level.
...
\gls{RS232} communication layer for integration with external equipment.
...
\emph{Phase 3 --- \gls{RS232} Communication:}

Design and implementation of an \gls{RS232} communication layer for exchange of
commands and data with external equipment, including interface configuration,
...
\begin{itemize}
      ...
      \item Implement an \gls{RS232} communication layer for exchange of commands and data
...
\begin{table}[htbp]
      ...
      \begin{tabular}{|p{3cm}|p{8cm}|c|c|}
            ...
            \hline
            \gls{RS232} Communication                                            &
            Design and implementation of an \gls{RS232} communication layer for
            exchange of commands and data with external equipment                &
            3 weeks                                                              & May 27 -- Jun 16                    \\
            \hline
            Integration, Testing \& Documentation                                &
            Integration of all components; system testing and validation;
            preparation of technical documentation                               &
            5 weeks                                                              & Jun 17 -- Jul 17                    \\
            \hline
      \end{tabular}
\end{table}
...
The scope of the internship evolved in one notable way relative to the
original plan.

The first was a minor documentation discrepancy rather than an actual change
in scope: the original internship proposal referred to the serial
communication interface as \gls{RS485}, but the external equipment in use at
\gls{GMV} communicated over \gls{RS232} from the outset. This was a misnomer
in the proposal document, arising from miscommunication between the involved
parties, and not an intentional design choice or a change in requirements. As
this was identified early, before any protocol-specific implementation work
took place, no requirement or design decision was actually changed, and it had
no impact on the development of the \gls{RS232} interface described in this
report.

The second change was more significant in terms of effort. As the developed
components matured, it became necessary to integrate them with existing
proprietary systems at \gls{GMV} \textcolor{red}{that had not been part of the
      original scope}. This integration work was not planned at the outset and
resulted in additional unplanned effort, reflected in the actual work plan in
