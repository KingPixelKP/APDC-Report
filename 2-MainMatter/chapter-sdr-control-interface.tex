...
The \gls{sdr-control-system} was implemented using a modular architecture,
with separate components responsible for device management, signal playback,
synchronization, communication, and hardware abstraction. Each component has a
well-defined responsibility, allowing the different parts of the system to
evolve independently while minimizing the impact of changes across the
codebase. This modular organization also simplifies integration with external
systems and facilitates testing by reducing unnecessary dependencies between
components.

A design goal of the internship was to integrate this system with other
components, that will be described in \autoref{cha:integration-testing}, to provide a complete solution for \gls{GNSS} signal
generation. Thus the system was designed with a client-server architecture,
where the control system acts as a server that listens for commands from client
applications and executes them accordingly. The communication between the
server and client is handled through a \gls{gRPC} interface, which allows for
remote procedure calls and provides a structured way to manage the interactions
between the different components of the system.

The architecture was also designed with software engineering concerns such as
maintainability, extensibility, and separation of concerns in mind. Hardware
specific functionality is encapsulated within the \texttt{dt\_facade}
...
for communication between the server and client, and it supports reflection.
The latter proved particularly valuable during development, enabling automatic
configuration parsing, simplified debugging, and the discovery of available
commands and their parameters without additional documentation, reducing the
barrier to entry for users unfamiliar with the system.

Another important design decision was the adoption of an
\textit{errors-as-values} model instead of exceptions. Besides avoiding the
runtime overhead associated with exception handling, this approach makes failure
explicit in function interfaces, encouraging callers to handle errors
deliberately and resulting in control flow that is easier to reason about,
test, and maintain.

Another significant decision was the choice of build system. While CMake\footnote{CMake is a powerful yet complex
build system.\cite{cmake}} is the standard for C++ projects, XMake was selected due to its more concise build
configuration, integrated package management, and build cache mechanism, which
reduced project complexity and improved incremental build performance when
compared to traditional CMake and Ninja\footnote{Ninja is a
small build system with a focus on speed.\cite{ninja-build}} based workflows.
\cite{xmake}\cite{xmake-cache}
...
through the \gls{SDR} hardware. Since such component was still unfinished a
